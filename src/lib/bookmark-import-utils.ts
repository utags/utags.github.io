...
export function validateBookmarks(bookmarksStore: BookmarksStore) {
  try {
    ...
    if (
      ...
      typeof data.meta.created !== 'number'
    ) {
      throw new TypeError('meta字段缺少必要属性')
    }
    ...
    for (const [url, bookmark] of Object.entries(data.data)) {
      ...
      if (bookmark.meta.created > 9_999_999_999_999) {
        noCreated++
      }
    }
...
export function htmlToBookmarks(html: string): BookmarksStore {
  ...
  const processFolder = (
    ...
    for (const item of items) {
      ...
      console.log(
        `Processing bookmark: ${title}, URL: ${href.slice(0, 50)}, Tags: ${tags.join(', ')}`
      )
      ...
      if (existingBookmark) {
        ...
        // 选择更早的创建日期
        const created = Math.min(existingBookmark.meta.created, addDate * 1000)
        ...
        // 选择修改日期更晚的标题
        const selectedTitle =
          updated === lastModified * 1000 ? title : existingBookmark.meta.title
